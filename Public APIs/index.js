...
app.get("/", async (req, res) => {
  try {
    ...
    res.render("index.ejs", { categories: categories.data.categories, categorizedAPI : categorizedAPI });
  } catch (error) {
    res.render("index.ejs", { APIs: JSON.stringify(error.response) });
  }
});

/* 原网站给了单独的catagory，可以直接获取，但也有以下字符串去重方法：
